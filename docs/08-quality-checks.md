...
| `npm run verify:team-users:dataconnect` | En emulateur, cree un profil SQL `User` local puis le relit via `ListUsers`; ne provisionne rien en sandbox. | Ajoute post-audit. |
| `npm run verify:email:dataconnect` | En emulateur, cree un `EmailThread`, un `EmailMessage` et une `EmailAttachment`, classe le fil, puis relit le detail et les listes. | Ajoute apres raccordement Email. |
| `npm run verify:documents:dataconnect` | En emulateur, cree un `DocumentFolder` et un `DocumentAttache`, puis relit `storagePath`, `tailleBytes` et `sha256`. | Ajoute apres durcissement Documents. |
| `npm run verify:planning:dataconnect` | En emulateur, cree un `PlanningEvent` et une `PlanningAssignment`, modifie titre/equipe/statut/creneau/notes via `UpdatePlanningEventDetails`, annule via `CancelPlanningEvent`, puis relit la carte par periode. | Ajoute apres raccordement Planning. |
