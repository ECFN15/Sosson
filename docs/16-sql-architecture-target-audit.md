...
| Planning | Partiel local: tables + operations + adapter + page en lecture/creation/modification/annulation SQL, pas sandbox | Page peut lire les evenements SQL par periode, creer, modifier, deplacer et annuler une carte SQL | SQL si source Data Connect, sinon localStorage fallback | P1 |
...
| Equipe | Partiel: lecture `ListUsers` des profils applicatifs | Profils SQL affiches si disponibles; equipes, membres, conges et matrice droits restent localStorage | localStorage + roles TS | P2 |
...
| `Client` | Client canonique operationnel ou client historique Excel. | 1-n `Chantier`, 1-n `ClientAlias`, 1-n `DocumentAttache`. | Seed operationnel, seed previsionnel, mutations futures. | Clients, Dashboard, Chantiers, Documents, Previsionnel, Statistiques. | `origineImport` separe operationnel/previsionnel. | Pas d'audit, soft delete, fusion/alias operationnels, dedoublonnage durable. |
| `Chantier` | Dossier operationnel ou chantier historique Excel. | n-1 `Client`, n-1 `User`, 1-n `Facture`, `DocumentAttache`, `PrevisionnelLine`. | Seed operationnel, seed previsionnel. | Chantiers, Detail chantier, Dashboard, Factures, Planning cible. | Relations principales presentes. | Pas d'historique statut, pas de planning SQL, pas de dateModification/audit. |
| `Facture` | Facture fournisseur rattachee a un chantier. | n-1 `Chantier`, optional docs via `DocumentAttache.facture`. | Seed operationnel, mutations `CreateFacture`, `SetFactureStatut`. | Factures, Detail chantier, Dashboard/statistiques. | Modele facture de base present. | Pas de fournisseur canonique, pas de piece jointe obligatoire, pas d'historique statut, pas de paiement/echeance. |
...
| Planning | Tables et operations SQL preparees localement. La page lit les evenements SQL par periode si disponible, cree les nouvelles cartes en SQL en mode Data Connect, modifie/deplace via `UpdatePlanningEventDetails`, et annule via `CancelPlanningEvent` sans suppression physique. |
...
| Detail chantier | Synthese, factures, documents statiques, emails store, planning local, badges de source par bloc, changement de statut. | `useOperationalData` pour chantier/factures; mutation SQL `UpdateChantierStatut` si source Data Connect active; fallback local explicite sinon; seeds/media locaux pour blocs non branches. | `Chantier`, `Facture`, `DocumentAttache`, `EmailThread`, `PlanningEvent`, `Rapport`. | `GetChantier`, `ListFactures`, `ListDocumentsByChantier`, `ListEmailThreadsByChantier`, `ListPlanningEventsByChantier`. | `UpdateChantierStatut`, mutations documents/factures/emails liens/planning. | Encore hybride; les blocs documents/emails/planning restent indicatifs tant que les queries par chantier ne sont pas branchees. | P1 |
...
| Emails | Boite Outlook locale, filtres, messages, badge de source SQL/Graph/fallback. | SQL partiel si disponible; sinon serveur Graph local `localhost:8787` ou seed fallback. | `EmailThread`, `EmailMessage`, `EmailAttachment`, `DocumentAttache`. | `ListEmailThreads`, `GetEmailThread`, `ListUnreadEmailThreads`. | `UpdateEmailThreadStatusAndLinks`, `CreateEmailThread`, `CreateEmailMessage`, `CreateEmailAttachment`, future sync Graph serveur. | Sync Graph serveur durable manquante; corps email lourd a garder hors SQL; Storage pieces jointes produit non finalise. | P1 |
| Planning | Calendrier d'interventions avec badge de source SQL/fallback. | SQL partiel si source Data Connect; sinon localStorage + chantiers store. | `PlanningEvent`, `PlanningAssignment`. | `ListPlanningEventsByPeriod`, `ListPlanningEventsByChantier`. | `CreatePlanningEvent`, `CreatePlanningAssignment`, `UpdatePlanningEventStatus`, `UpdatePlanningEventDetails`, `CancelPlanningEvent`. | Conflits de planning non detectes, pas d'historique dedie par evenement, annulation limitee a un statut soft. | P1 |
...
| Equipe | Profils applicatifs SQL, membres, roles, absences, matrice acces. | `ListUsers` si SQL Connect repond; equipes, membres, absences et matrice acces restent localStorage. | `User`, future `TeamMember`, `Absence`, `AccessPolicy` ou RBAC explicite. | `ListUsers`, futur `GetUser`, `ListTeamAbsences`. | Futures mutations admin uniquement: `UpdateUserRole`, `SetUserActive`, `CreateAbsence`. | Divergence entre UI equipe et RBAC serveur; ne pas faire croire que localStorage change les vrais roles SQL. | P2 |
...
| Qu'est-ce qui est pret a etre seede ? | `dataconnect/previsionnel_seed_data.gql` et chunks `dataconnect/previsionnel_seed/*.gql`, plus seed operationnel. |
...
- Creation SQL de nouvelles cartes avec assignment; fallback localStorage uniquement quand SQL n'est pas la source active.
- Edition et deplacement SQL branches via `UpdatePlanningEventDetails`; annulation soft branchee via `CancelPlanningEvent`, avec statut `cancelled` et sans suppression physique.
...
- Page Rapports dediee: lecture `ListRapports`, lecture du dernier `AnalyticsSnapshot`, creation d'un brouillon `Rapport` metadata si SQL Connect est disponible.
...
| `npm run verify:team-users:dataconnect` | OK via checkpoint emulateur: creation/relecture d'un profil SQL `User` local via `ListUsers`; preuve `tmp/checkpoint-002/team-users-local.json`. |
...
| `npm run verify:documents:dataconnect` | OK via emulateur: creation d'un `DocumentFolder` + `DocumentAttache`, relecture `storagePath`, `tailleBytes` et `sha256`; preuve `tmp/checkpoint-002/documents-local.json`. |
| `npm run verify:planning:dataconnect` | OK via emulateur: creation d'un `PlanningEvent` + `PlanningAssignment`, modification via `UpdatePlanningEventDetails`, annulation via `CancelPlanningEvent`, relecture par periode; preuve `tmp/checkpoint-002/planning-local.json`. |
