...
|---|---|---|
| QA locale | `checkpoint:002:local` vert le 2026-05-18 avec `check:operational-lifecycle-readiness` dans `ci:sandbox`. | Pret localement, hors validation sandbox distante. |
...
| Cycle nouveau client operationnel | Preuve locale `operational-lifecycle-local.json` verte en emulateur et relue par `npm run check:operational-lifecycle-proof`. | Pret techniquement localement, scenario metier a confirmer. |
...
|---|---|---|---|
| Lire les docs de depart | `AGENTS.md`, `documentation.md`, `docs/00-index.md`, `docs/05-sql-connect.md`, `docs/12-ai-agent-roadmap.md`, `docs/13-checkpoint-002-readiness.md` | Lues avant patchs. | Aucune. |
...
| Cycle nouveau client reel | `scripts/verify-operational-lifecycle-dataconnect-local.mjs`, `scripts/check-operational-lifecycle-proof.mjs`, `tmp/checkpoint-002/operational-lifecycle-local.json` | En emulateur: profil `User` local autorise, prospect sans chantier, devis demande sans chantier, client `operationnel`, chantier rattache, devis signe, factures fournisseur definitives/categorisees, relectures operationnelles et absence de fuite previsionnelle; la preuve JSON est verifiee par `npm run check:operational-lifecycle-proof`. | Relancer le checkpoint emulateur apres toute evolution du domaine Devis ou de la promotion previsionnel -> operationnel. |
...
| QA/CI | `.github/workflows/sandbox-checks.yml`, `package.json` | `ci:sandbox` vert localement le 2026-05-18 et inclut les tests data-state + garde-fous documentation/secrets + lifecycle readiness. | CI distante verte sur PR/push. |
...
npm run verify:chantier-status:dataconnect -- --output=tmp/checkpoint-002/chantier-status-local.json
npm run verify:client-update:dataconnect -- --output=tmp/checkpoint-002/client-update-local.json
...
npm run verify:previsionnel-edits:dataconnect -- --output=tmp/checkpoint-002/previsionnel-edits-local.json
npm run verify:factures:dataconnect -- --output=tmp/checkpoint-002/factures-local.json
npm run verify:operational-lifecycle:dataconnect -- --output=tmp/checkpoint-002/operational-lifecycle-local.json
...
Resultat observe le 18 mai 2026 apres ajout du cycle operationnel client/chantier/factures:
...
Preuves locales ecrites dans `tmp/checkpoint-002/counts-local.json`, `tmp/checkpoint-002/operational-boundary-local.json`, `tmp/checkpoint-002/chantier-status-local.json`, `tmp/checkpoint-002/client-update-local.json`, `tmp/checkpoint-002/operational-lifecycle-local.json`, `tmp/checkpoint-002/team-users-local.json`, `tmp/checkpoint-002/email-local.json`, `tmp/checkpoint-002/factures-local.json`, `tmp/checkpoint-002/documents-local.json`, `tmp/checkpoint-002/planning-local.json`, `tmp/checkpoint-002/report-local.json`, `tmp/checkpoint-002/previsionnel-edits-local.json`, `tmp/checkpoint-002/analytics-snapshot-local.json` et `tmp/checkpoint-002/checkpoint-audit-local.json`.

## Decisions metier avant sandbox

Fiche source: `docs/17-operational-lifecycle-scenario.md`.

Les 9 decisions metier sont renseignees et remplacent l'ancien scenario technique simple `nouveau client -> chantier -> factures`:

1. fiche client/prospect des le premier contact;
2. client possible sans chantier et creation combinee possible;
3. vrai domaine `Devis` avec `devis_demande`, `devis_envoye`, `devis_signe`;
4. factures fournisseur saisies pendant chantier par plusieurs roles;
5. toute facture fournisseur importee impacte dashboard/statistiques;
6. previsionnel Excel semi-operationnel a rapprocher/promouvoir;
7. statuts chantier valides: `prospect`, `devis_a_faire`, `devis_envoye`, `signe`, `en_preparation`, `en_cours`, `en_pause`, `termine`, `cloture`, `annule`;
8. KPI gerant: budget, depenses, marge, retard, productivite salarie;
9. Moteur live attendu comme outil full page type Access: base, diagramme metier, explorateur client/chantier/facture/devis.

La conclusion correcte depend maintenant des validations locales/emulateur: il faut une preuve JSON lifecycle contenant Devis et prospect sans chantier avant de demander une validation sandbox.

Pour appliquer les reponses sans toucher a la sandbox:
...
- les 9 reponses metier de `docs/17-operational-lifecycle-scenario.md` sont completees, `npm run check:operational-lifecycle-decisions` est OK et le lifecycle local a ete relance si le parcours change;
