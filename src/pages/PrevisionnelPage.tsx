...
export function PrevisionnelPage() {
  ...
  const spreadsheet = useMemo(
    () => buildSpreadsheetView(currentPrevisionnelSheet, {}, edited),
    [edited],
  )
  ...
  return (
    <div className="min-h-full bg-[#FAF6F2] p-6 xl:p-8">
      <div className="mb-5 flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
        <div>
          ...
          <p className="mt-2 max-w-3xl text-sm text-[#6B6B6B]">
            Interface de saisie web sur la structure du prévisionnel Excel. Le tableur complet s'ouvre maintenant en plein écran pour utiliser tout l'espace disponible.
          </p>
        </div>
      </div>
      ...
      <div className="mb-5 grid gap-4 xl:grid-cols-4">
        {[
          ['CA prévision', euro(caPrevision), 'Colonne D recalculée'],
          ['CA contrat', euro(caContrat), 'Colonne E recalculée'],
          ['Prévu mensuel', euro(monthlyPlanned), 'Somme AR:BO recalculée'],
          ['Cellules modifiées', editedCount.toLocaleString('fr-FR'), 'Sauvegarde dans la page tableur'],
      ...
      <div className="mb-4 flex flex-wrap items-center gap-3 rounded-[14px] border border-[#F2E8DC] bg-white px-4 py-3 text-[12px] text-[#6B6B6B]">
        ...
        <span className="ml-auto inline-flex items-center gap-2 font-semibold text-[#1E1E1E]">
          ...
          Sauvegarde SQL disponible dans le tableur plein écran
        </span>
      </div>
